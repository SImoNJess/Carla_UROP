...
import sys
import time
carla_root = 'D:/UE4-Carla/sim/PythonAPI/carla'
...
from agents.tools.misc import distance_vehicle, is_within_distance # type:ignore
...
if current != "Town06_Opt":
    # only reload if it’s not already Town04
    world = client.load_world("Town06_Opt")
    print("Loaded map:", world.get_map().name)
else:
    print("Already on map:", current)
map = world.get_map()
